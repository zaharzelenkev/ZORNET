...
if st.session_state.page == "Главная":
    ...
    # ЕСЛИ ПОИСКА НЕТ - ПОКАЗЫВАЕМ ГЛАВНУЮ
    else:
        # БЫСТРЫЕ ДЕЙСТВИЯ
        ...
        with quick_actions_cols[0]:
            st.markdown("""
            <div style="background: white; border: 1px solid #dee2e6; border-radius: 16px; padding: 25px; margin: 20px 0; text-align: center; transition: all 0.3s;">
                <div style="font-size: 32px; margin-bottom: 15px;">🤖</div>
                <div style="font-weight: 600; margin-bottom: 10px;">ZORNET AI</div>
                <p style="color: #666; font-size: 14px;">Искусственный интеллект для ответов на любые вопросы</p>
            </div>
            """, unsafe_allow_html=True)

            if st.button("Задать вопрос", key="quick_ai", use_container_width=True):
                st.session_state.page = "ZORNET AI"
                st.rerun()
        ...
        with quick_actions_cols[1]:
            ...
            <div style="background: white; border: 1px solid #dee2e6; border-radius: 16px; padding: 25px; margin: 20px 0; text-align: center; transition: all 0.3s;">
                <div style="font-size: 32px; margin-bottom: 15px;">📰</div>
                <div style="font-weight: 600; margin-bottom: 10px;">Новости</div>
                <p style="color: #666; font-size: 14px;">Последние новости Беларуси и мира</p>
            </div>
            """, unsafe_allow_html=True)

            if st.button("Читать новости", key="quick_news", use_container_width=True):
        ...
        with quick_actions_cols[2]:
            ...
            <div style="background: white; border: 1px solid #dee2e6; border-radius: 16px; padding: 25px; margin: 20px 0; text-align: center; transition: all 0.3s;">
                <div style="font-size: 32px; margin-bottom: 15px;">💾</div>
                <div style="font-weight: 600; margin-bottom: 10px;">Облако</div>
                <p style="color: #666; font-size: 14px;">Ваши файлы в безопасном хранилище</p>
            </div>
            """, unsafe_allow_html=True)

            if st.button("Открыть Диск", key="quick_disk", use_container_width=True):
        ...
        for idx, query in enumerate(popular[:5]):
            with popular_cols[idx]:
                if st.button(query, key=f"pop_{idx}", use_container_width=True):
                    st.session_state.search_query = query
                    st.rerun()

        popular_cols2 = st.columns(5)
        for idx, query in enumerate(popular[5:]):
            with popular_cols2[idx]:
                if st.button(query, key=f"pop_{idx + 5}", use_container_width=True):
                    st.session_state.search_query = query
                    st.rerun()

        st.markdown("---")

        # ПРОСТОЙ ЧАТ НА ГЛАВНОЙ
        st.markdown("### 💬 Быстрый вопрос ZORNET")
        quick_question = st.text_input("Задайте быстрый вопрос...", key="quick_question")

        if quick_question and st.button("Спросить", key="ask_quick"):
            with st.spinner("ZORNET думает..."):
                try:
                    ...
                    st.markdown(f"""
                    <div style="background: #FFFBF0; border-left: 4px solid #D4AF37; 
                                padding: 15px; border-radius: 8px; margin-top: 10px;">
                        <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;">
                            <div style="width: 24px; height: 24px; background: #D4AF37; 
                                        border-radius: 50%; display: flex; align-items: center; 
                                        justify-content: center; color: white; font-weight: bold;">
                                Z
                            </div>
                            <div style="font-weight: 600; color: #B8860B;">ZORNET AI</div>
                        </div>
                        <div style="color: #333;">{response}</div>
                    </div>
                    """, unsafe_allow_html=True)
                except:
                    st.error("Сейчас я перегружен. Попробуйте позже.")
